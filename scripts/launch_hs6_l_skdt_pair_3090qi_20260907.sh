...
common=(
  ...
  GRAD_ACCUM_STEPS=4
  EXPECTED_GLOBAL_BATCH_SIZE=1024
...
true_output=$REPO/outputs/01_training_runs/HS6_L_e11to15_SKDT_stable_w0.5_bs64_gb1024_seed0_4x3090qi_${STAMP}
shuffle_output=$REPO/outputs/01_training_runs/HS6_L_e11to15_SKDT_shuffled_stable_w0.5_bs64_gb1024_seed0_4x3090qi_${STAMP}
log_root=$REPO/outputs/auto_train_logs
mkdir -p "$log_root"

env "${common[@]}" \
  GPU_GROUP=0,1,2,3 MASTER_PORT=31911 TARGET_MODE=stable_delta \
  RUN_TAG=$(basename "$true_output") OUTPUT_DIR="$true_output" \
  "$LAUNCHER" >"$log_root/$(basename "$true_output").log" 2>&1 &
true_pid=$!

env "${common[@]}" \
  GPU_GROUP=4,5,6,7 MASTER_PORT=31912 TARGET_MODE=shuffled_stable_delta \
