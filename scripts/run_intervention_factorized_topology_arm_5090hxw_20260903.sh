#!/usr/bin/env bash
# Run one compute-matched pure-SSL intervention-factorization arm on four GPUs.
set -euo pipefail

REPO=${REPO:-/home/xzj/biodino}
PYTHON_BIN=${PYTHON_BIN:-/home/xzj/miniconda3/envs/dinov3/bin/python}
CONFIG=${CONFIG:-$REPO/dinov3/configs/train/microscopy_continual_vitl16_intervention_factorized_topology.yaml}
...
cmd=(
  ...
  optim.trainable_last_blocks=4
  "student.resume_from_teacher_chkpt=$MATURE_CKPT"
  intervention_factorized_topology.enabled=true
  intervention_factorized_topology.mode="$IFT_MODE"
  ...
  acquisition_orbit_deflation.enabled=false
...
printf '[ift] arm=%s mode=%s GPUs=%s batch=4x%sx%s=%s updates=%s output=%s\n' \
  "$ARM" "$IFT_MODE" "$GPU_GROUP" "$BATCH_SIZE_PER_GPU" "$GRAD_ACCUM_STEPS" \
