...
def model_color(model: str, group: str) -> str:
    if group == 'ours' or model == 'BioDINOv3':
        return OURS
    palette = {
        'GigaPath': '#B99BCC',
        'Virchow2': '#79CDBF',
    ...
    return palette.get(model, EXT if group == 'best_external' else MUTED)


def plot_metric(rows, title, ylabel, name, higher=True, ylim=None, value_fmt='{:.3f}'):
    # rows: list of (model_display, score, group). Render as model on x-axis, metric on y-axis.
    rows = [(m, s, g) for m, s, g in rows if s is not None]
    rows.sort(key=lambda x: x[1], reverse=higher)
    labels = [r[0] for r in rows]
    scores = [r[1] for r in rows]
    groups = [r[2] for r in rows]
    colors = [model_color(m, g) for m, _, g in rows]

    w = max(9.5, min(15.5, 0.62 * len(rows) + 4.0))
    fig, ax = plt.subplots(figsize=(w, 5.8))
    x = np.arange(len(labels))
    bars = ax.bar(x, scores, color=colors, edgecolor='#4A4A4A', linewidth=0.65)
    ...
    if ylim:
        ...
        pad = (hi - lo) * 0.16 if hi != lo else abs(hi) * 0.08 + 0.01
        if lo >= 0:
            ax.set_ylim(max(0, lo - pad * 0.4), hi + pad)
        else:
            ax.set_ylim(lo - pad, hi + pad)

    ymin, ymax = ax.get_ylim()
    dy = (ymax - ymin) * 0.012
    for bar, s, group in zip(bars, scores, groups):
        ax.text(
            bar.get_x() + bar.get_width()/2,
            s + dy if s >= 0 else s - dy,
            value_fmt.format(s),
            va='bottom' if s >= 0 else 'top',
            ha='center',
            fontsize=8.5,
            fontweight='bold' if group == 'ours' else 'normal',
            color=TEXT,
        )

    # Highlight ours label in red and bold.
    for tick, lab in zip(ax.get_xticklabels(), labels):
        if lab == 'BioDINOv3' or lab.startswith('L 1TB'):
            tick.set_color(OURS)
...
def readme():
    (OUT / 'README.md').write_text(
        '# Task-wise foundation-model figures, vertical white style\n\n'
