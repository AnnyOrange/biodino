...
# 路径配置
OUTPUT_DIR = "/mnt/huawei_deepcad/onepb/shards_task_highNA_file"
REPORT_FILE = "task2_hybrid_report_highNA_file.txt"
...
def main():
    ...
    with open(REPORT_FILE, "w", encoding="utf-8") as f_log:
        ...
        log(f"Strategy: OPTION C - FILE-ATOMIC SHARDING (Frame Indexing)")
        ...
        log(f"  【File-Atomic Sharding】每个文件的所有帧必须在同一个 shard 中")
        log("-" * 60)
        ...
        # ============================================
        # PHASE 2: WRITE SHARDS (WITH FILE-ATOMIC SPLITTING)
        # ============================================
        log(f"Phase 2: Writing shards with File-Atomic Splitting (每个文件的所有帧保持在同一个 shard)...")
        ...
        def write_expanded_shards(entries, shard_type, group_h, group_w, bucket_key=None):
            ...
            【核心改动】File-Atomic Sharding:
            - 以"文件"为单位进行 append
            - 如果当前 shard 放不下某个文件的所有帧，就先封存当前 shard，在新 shard 中放入该文件
            - 如果单个文件就超过 MAX_SHARD_SIZE，它会独占一个 shard (允许溢出)
            """
            nonlocal stats
            ...
            for f_path, f_count, f_h, f_w, channel_idx, channel_count in entries:
                ...
                # 2. 【核心修改】检查是否需要新开 Shard (File-Atomic Sharding)
                # 如果当前 shard 加上新文件会超过限制，且当前 shard 不为空，则先保存当前 shard
                # 注意：如果单个文件就超过 MAX_SHARD_SIZE，它会独占一个 shard (并在其中溢出，这是允许的)
                if current_shard_items and (len(current_shard_items) + len(file_items) > MAX_SHARD_SIZE):
                    # --- 保存当前 Shard ---
                    if shard_type == "standard":
                        # 使用当前 shard 中第一个文件的 channel 信息来命名 (同一组内 channel 应该是一致的)
                        ref_ch = current_shard_items[0]["channel_idx"]
                        fname = f"task_{stats['total_shard_files']:06d}_std_h{group_h}_w{group_w}_ch{ref_ch+1}.json"
                    ...
                    meta = {
                        ...
                        "is_2p": True,
                        "channel_idx": int(current_shard_items[0]["channel_idx"]),
                        "expected_channel_count": int(current_shard_items[0]["expected_channel_count"]),
                    }
                ...
                # 3. 将新文件的所有帧加入当前 Buffer
                current_shard_items.extend(file_items)
        ...
        end_time = time.time()
        log("\n" + "="*60)
        log(f"HIGHNA FILE-ATOMIC SHARDING SUMMARY")
        log("="*60)
