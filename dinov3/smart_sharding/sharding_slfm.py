...
# 路径配置
OUTPUT_DIR = "/mnt/huawei_deepcad/onepb/shards_task_slfm"
REPORT_FILE = "task2_hybrid_report_slfm.txt"

# --- 2. 核心参数 (Core Parameters) ---
# 【重要修改】现在的 SHARD_SIZE 指的是"帧数/Frame Count"，即 JSON list 的长度
...
def get_sql_query():
    ...
    WHERE file_path LIKE '%slfm%'
      AND file_path LIKE '%capture/C%'
      AND channel_count = 1
...
def write_shard_file(data, meta, filename):
    ...
    # meta 中的 count 和 rows 现在是相等的，因为我们展开了
...
def main():
    ...
    # --- 准备 buffer ---
    # 【修改点】Value 存储结构改为: [(path, count, h, w), ...]
    # 我们需要保存 h, w，因为 Misc 桶里不同文件的尺寸不一样，展开后需要把尺寸写进 JSON
    standard_groups = {} 
    ...
    with open(REPORT_FILE, "w", encoding="utf-8") as f_log:
        ...
        log(f"  Filters:")
        log(f"    - file_path LIKE '%slfm%' AND file_path LIKE '%capture/C%'")
        log(f"    - channel_count = 1")
        log("--- THRESHOLDS ---")
        ...
        try:
            ...
            cursor = conn.cursor(name='task2_slfm_split_cursor')
        ...
        current_group_buffer = [] # Stores (f_path, img_count, h, w)
        ...
        while True:
            ...
            for row in rows:
                ...
                if group_key != current_group_key:
                    ...
                    # Reset
                    # 【修改点】Buffer 中包含 h, w
                    current_group_buffer = [(f_path, img_count, h, w)]
                    current_group_frame_count = img_count
                    current_group_key = group_key
                else:
                    current_group_buffer.append((f_path, img_count, h, w))
                    current_group_frame_count += img_count
        ...
        def write_expanded_shards(entries, shard_type, group_h, group_w, bucket_key=None):
            """
            entries: list of (file_path, count, h, w)
            ...
            # 遍历每一个文件
            for f_path, f_count, f_h, f_w in entries:
                ...
                for i in range(f_count):
                    ...
                    item = {
                        "path": f_path,
                        "frame_idx": i, # 核心：帧索引
                        # 仅在 Misc 桶或为了保险起见，写入原始尺寸
                        # Standard 桶其实不需要这个(因为meta里有)，但加上也没坏处，保持统一方便处理
                    ...
                    if len(current_shard_items) >= MAX_SHARD_SIZE:
                        ...
                        if shard_type == "standard":
                            fname = f"task_{stats['total_shard_files']:06d}_std_h{group_h}_w{group_w}.json"
                            meta_h, meta_w = group_h, group_w
                        else:
                            fname = f"task_{stats['total_shard_files']:06d}_misc_{bucket_key}.json"
                            meta_h, meta_w = -1, -1 # Misc 桶没有统一尺寸
                        ...
                        meta = {
                            ...
                            "height": meta_h,
                            "width": meta_w,
                            "count": len(current_shard_items), # 这里的 count 就是 item 数量
                            "is_2p": True  # 标记这是 2P 数据，需要按帧读取
                        }
        ...
        for (h, w), entries in standard_groups.items():
        ...
        end_time = time.time()
        log("\n" + "="*60)
        log(f"SLFM SPLIT SHARDING SUMMARY")
        log("="*60)
