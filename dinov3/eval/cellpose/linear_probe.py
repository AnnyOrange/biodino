...
"""
Cellpose 16-bit 细胞分割 Linear Probing 验证脚本
...
使用方法:
    python -m dinov3.eval.cellpose.linear_probe \
        ...
        --output-dir /mnt/huawei_deepcad/dinov3/dinov3/outputs/cellpose_linear_probe \
        --epochs 10 \
        --batch-size 8
"""
...
logger = logging.getLogger("cellpose_linear_probe")
...
class CellposeLinearProbeDataset(Dataset):
    """
    Cellpose 16-bit 图像分割数据集
    
    支持读取 16-bit PNG 图像并应用指定的预处理方法
    """
    ...
    def __init__(
        ...
        if self.size is None:
            sample_img = cv2.imread(self.img_paths[0], cv2.IMREAD_UNCHANGED)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 5. 加载 Mask 并转换为二值 (0:背景, 1:细胞)
        mask = cv2.imread(self.mask_paths[idx], cv2.IMREAD_UNCHANGED)
        if mask is None:
            raise ValueError(f"无法读取掩码: {self.mask_paths[idx]}")
...
def get_dataset_paths(data_root: str, split: str = 'train') -> Tuple[List[str], List[str]]:
    ...
    logger.info(f"[{split}] 找到 {len(img_paths)} 张图像和 {len(mask_paths)} 张掩码")
...
def run_experiment(
    ...
    # 创建数据集
    train_dataset = CellposeLinearProbeDataset(
        train_img_paths, train_mask_paths,
        mode=mode, size=(224, 224), augment=True
    )
    test_dataset = CellposeLinearProbeDataset(
...
def main():
    parser = argparse.ArgumentParser(description='Cellpose 16-bit Linear Probing')
    ...
    test_img_paths, test_mask_paths = get_dataset_paths(args.data_path, 'test')
