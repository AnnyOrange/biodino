...
"""
CSC (Cell Segmentation Challenge) Zero-Shot PCA 可视化
...
logger = logging.getLogger("csc_pca")
...
def load_and_preprocess_image(
    ...
    mode: str = 'hybrid',
    patch_size: int = 16,
) -> Tuple[torch.Tensor, Tuple[int, int], np.ndarray, Dict[str, str]]:
    """
    加载并预处理 CSC 图像（支持多种格式和位深）
    
    CSC 图像格式多样：tif, tiff, png, bmp, jpg
    位深：8-bit 或 16-bit
    ...
    Returns:
        ...
        img_info: 图像信息字典 {'format': 'PNG', 'bit_depth': '8-bit', 'channels': 'Multi (RGB)'}
    """
    # 使用多格式加载函数
    img = load_image_multi_format(img_path)
    ...
    # 获取图像格式
    img_format = os.path.splitext(img_path)[1].upper().replace('.', '') or 'UNKNOWN'
    ...
    if len(original_shape) == 2:
        ...
        if original_shape[2] == 1:
            ...
            channels = f"Multi ({original_shape[2]}C)"
    else:
        channels = "Unknown"
    ...
    return tensor, original_size, preprocessed_np, img_info
...
def select_diverse_samples(
    ...
    要求：
    1. 确保有至少 1/4 的 16-bit 图像
    2. 确保格式包含 tif, tiff, png, bmp 至少各一个（如果存在）
    3. 其余随机选择
    
    Args:
        img_paths: 所有图像路径
        mask_paths: 所有标签路径（与 img_paths 一一对应）
        max_samples: 最大样本数
        seed: 随机种子
    
    Returns:
        选中的 (img_paths, mask_paths)
    """
    import random
    ...
    # 1. 分析所有图像的特征
    image_features = []
    logger.info("分析图像特征以选择多样化样本...")
    
    for img_path in tqdm(img_paths, desc="Analyzing images", leave=False):
        ext = os.path.splitext(img_path)[1].lower()
        # 快速检查是否为16-bit（只读取一小部分来检查dtype）
        is_16bit = False
        try:
            if ext in ['.tif', '.tiff']:
                if tifffile is not None:
                    # 对于TIFF，尝试读取第一个slice或第一帧
                    try:
                        img_sample = tifffile.imread(img_path, key=0)
                        # 如果是3D，取第一个slice
                        if len(img_sample.shape) > 2:
                            img_sample = img_sample[0] if img_sample.shape[0] < 100 else img_sample[:, :, 0]
                        is_16bit = img_sample.dtype == np.uint16 if img_sample is not None else False
                    except:
                        # 如果key=0失败，读取全部（可能很慢但准确）
                        img_sample = tifffile.imread(img_path)
                        if len(img_sample.shape) > 2:
                            img_sample = img_sample[0] if img_sample.shape[0] < 100 else img_sample[:, :, 0]
                        is_16bit = img_sample.dtype == np.uint16 if img_sample is not None else False
            else:
                # 对于其他格式，使用cv2读取（只读取第一个像素行来检查）
                img_sample = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)
                if img_sample is not None:
                    if len(img_sample.shape) == 3:
                        img_sample = img_sample[0, 0, :]  # 取第一个像素
                    is_16bit = img_sample.dtype == np.uint16 if img_sample is not None else False
        except Exception:
            # 如果读取失败，假设是8-bit
            pass
    ...
    for feat in image_features:
        ...
        if fmt not in format_groups:
            format_groups[fmt] = []
        format_groups[fmt].append(feat['path'])
        
        if feat['is_16bit']:
            bit16_images.append(feat['path'])
        else:
            bit8_images.append(feat['path'])
    
    # 3. 选择样本
    selected = set()
    
    # 3.1 确保每种格式至少有一个（如果存在且还需要更多样本）
    required_formats = ['tif', 'tiff', 'png', 'bmp']
    format_selected_count = 0
    for fmt in required_formats:
        if fmt in format_groups and len(format_groups[fmt]) > 0 and format_selected_count < max_samples:
            candidate = random.choice(format_groups[fmt])
            if candidate not in selected:
                selected.add(candidate)
                format_selected_count += 1
    
    # 3.2 确保有 1/2 的 16-bit 图像
    n_16bit_needed = max(1, max_samples // 2) - len([p for p in selected if p in bit16_images])
    available_16bit = [p for p in bit16_images if p not in selected]
    if n_16bit_needed > 0 and len(available_16bit) > 0:
        n_16bit_needed = min(n_16bit_needed, len(available_16bit))
        selected_16bit = random.sample(available_16bit, n_16bit_needed)
        selected.update(selected_16bit)
    
    # 3.3 随机填充剩余
    remaining_needed = max_samples - len(selected)
    available = [p for p in img_paths if p not in selected]
    if remaining_needed > 0 and len(available) > 0:
        remaining_needed = min(remaining_needed, len(available))
        selected_remaining = random.sample(available, remaining_needed)
        selected.update(selected_remaining)
    
    # 4. 转换为列表并保持顺序
    selected_list = [p for p in img_paths if p in selected]
    
    # 5. 获取对应的 mask
    img_to_mask = dict(zip(img_paths, mask_paths))
    selected_masks = [img_to_mask[p] for p in selected_list]
    
    logger.info(f"选择了 {len(selected_list)} 张图像:")
    logger.info(f"  - 16-bit: {len([p for p in selected_list if p in bit16_images])} 张")
    logger.info(f"  - 8-bit: {len([p for p in selected_list if p in bit8_images])} 张")
    format_counts = {}
    for p in selected_list:
        fmt = os.path.splitext(p)[1].lower().replace('.', '')
        format_counts[fmt] = format_counts.get(fmt, 0) + 1
    logger.info(f"  - 格式分布: {format_counts}")
    
    return selected_list, selected_masks
...
def save_pca_visualization(
    ...
    img_info: Optional[Dict[str, str]] = None,
):
    """
    保存 PCA 可视化结果
    ...
    title = f'Preprocessed ({mode})'
    if img_info is not None:
        info_str = f"{img_info.get('format', 'UNK')}, {img_info.get('bit_depth', 'UNK')}, {img_info.get('channels', 'UNK')}"
        title += f"\n{info_str}"
...
def save_comparison_figure(
    ...
    for i, mode in enumerate(modes):
        ...
        # 添加图像信息（只在第一列显示）：格式、位深、通道
        if i == 0 and img_info is not None:
            info_str = f"{img_info.get('format', 'UNK')}, {img_info.get('bit_depth', 'UNK')}, {img_info.get('channels', 'UNK')}"
            title += f"\n{info_str}"
...
def run_pca_visualization(
    ...
    # 智能选择多样化的样本
    img_paths, mask_paths = select_diverse_samples(img_paths, mask_paths, max_samples, seed=kmeans_seed)
    ...
    for idx in tqdm(range(n_samples), desc="Processing"):
        ...
        # 加载 mask（CSC 使用 TIFF 格式的 instance mask）
        instance_mask = load_mask_tiff(mask_path)
        # 转换为二值 mask（0: 背景, 1: 前景）
        mask = instance_mask_to_binary(instance_mask).astype(np.float32)
        
        # 对每种预处理方法
        results = {}
        current_kmeans_ious: Dict[str, float] = {}  # 当前图的 K-Means IoU
        img_info = None  # 图像信息（只需要获取一次）
        
        for mode in modes:
            # 加载并预处理图像
            img_tensor, original_size, preprocessed_np, img_info = load_and_preprocess_image(
            ...
            save_pca_visualization(
                ...
                mode, fg_mask_patches, img_info
            )
        
        # 保存对比图（传递 mask、K-Means IoU 和图像信息）
...
def main():
    parser = argparse.ArgumentParser(description='CSC Zero-Shot PCA Visualization')
    ...
    parser.add_argument('--split', type=str, default='train',
                        choices=['train', 'tune', 'test', 'test_public', 'test_hidden'], 
                        help='使用的数据集分割')
    ...
    # 获取数据路径（使用 CSC 数据集工具）
    img_paths, mask_paths = get_csc_dataset_paths(args.data_path, args.split)
