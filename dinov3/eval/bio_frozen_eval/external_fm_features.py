...
from .external_fm_protocol import MODELS,resolve_dense_layers
...
class RuleFMFeatures:
    def __init__(self,name,device='cuda:0',layers='last'):
        ...
        hashes=json.loads(Path(__file__).with_name('external_fm_source_hashes.json').read_text())
    ...
    def __call__(self,imgs):
        ...
        for index in self.layer_record['layer_indices']:
            ...
            if self.norm is not None:tokens=self.norm(tokens)
