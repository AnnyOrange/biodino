...
def audit_dense(dataset,benchmark,protocol=None,observation=False):
    ...
    expected={'conic':(3469,494,1018),'livecell':(3253,570,1564),
              'monuseg':(30,7,14),'multimodal_cellseg':(855,172,100)}
        ...
        if dataset=='tissuenet':
            ...
            with zipfile.ZipFile(path) as archive:
                with archive.open('X.npy') as handle:
                    ...
                    if version==(1,0):shape,_,_=np.lib.format.read_array_header_1_0(handle)
                    elif version==(2,0):shape,_,_=np.lib.format.read_array_header_2_0(handle)
                    else:raise ValueError(f'Unsupported official NPY header version {version}')
