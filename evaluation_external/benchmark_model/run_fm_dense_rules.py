...
sys.path.insert(0,os.environ.get('DINOV3_CODE_ROOT','/mnt/huawei_deepcad/dinov3'))

from dinov3.eval.bio_frozen_eval.external_fm_protocol import MODELS, SEGMENTATION, PANNUKE_PROTOCOLS, dense_recipe, validate_best_head
...
def main(argv=None):
    ...
    parser.add_argument('--split-protocol', choices=PANNUKE_PROTOCOLS)
    parser.add_argument('--extract-batch-size', type=int, default=32)
    parser.add_argument('--probe-batch-size', type=int, default=32)
    parser.add_argument('--probe-budget', type=int, choices=(20,50))
    parser.add_argument('--seed', type=int, choices=(0,1,2))
    parser.add_argument('--plan-only', action='store_true')
    args = parser.parse_args(argv)
    plans=[]
    for dataset in args.datasets:
        folds=(args.split_protocol,) if dataset=='pannuke' and args.split_protocol else PANNUKE_PROTOCOLS if dataset=='pannuke' else (None,)
    ...
    if (args.extract_batch_size,args.probe_batch_size)!=(32,32):
        parser.error('New dense experiments require explicit feature/probe batch 32; old batch-only results are reusable')

    import numpy as np
    import torch
    from benchmark_eval.rules_features import RuleFMFeatures
    from dinov3.eval.bio_segmentation.feature_extractor import _build_dataset
    from dinov3.eval.bio_segmentation.linear_probe import DATASET_CONFIGS, run_cached_linear_probe
    ...
    for plan in plans:
        ...
        cache.build_bioseg_dataset=lambda dataset,split,size: _build_dataset(
            ...
            augment=False,do_normalize=False,multichannel=plan['model'] in ('pe','jump_cp'),
        ...
        settings=SimpleNamespace(img_size=recipe['image_size'],device=args.device,feature_canonical=True,
            overwrite_cache=False,extract_batch_size=args.extract_batch_size,num_workers=2,max_feature_side=0)
        try:
        ...
        for budget in [args.probe_budget] if args.probe_budget else recipe['probe_budgets']:
            for seed in [args.seed] if args.seed is not None else recipe['seeds']:
