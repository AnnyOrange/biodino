    ...
    def encode_images(self, images):
        ...
        for image in images:
            if torch.is_tensor(image):
                tensor=image.detach().float().clamp(0.,1.)
                height,width=tensor.shape[-2:]
                scale=self.resize/min(height,width)
                new_h=max(self.size,round(height*scale));new_w=max(self.size,round(width*scale))
                tensor=F.interpolate(tensor.unsqueeze(0),size=(new_h,new_w),mode='bilinear',
                                     align_corners=False,antialias=True).squeeze(0)
                top=(new_h-self.size)//2;left=(new_w-self.size)//2
                prepared.append(tensor[:,top:top+self.size,left:left+self.size])
...
def main():
    ...
    from dinov3.eval.bio_frozen_eval.run_classification import resolve_image_size,resolve_dataset_resize_size
    ...
    for target in targets:
        target.update(checkpoint=str(probe.MODEL_REGISTRY[args.model].path),batch_size=64,seed=0,
            ...
            input_tensor_resize='bilinear-antialiased-matched-HS0',
